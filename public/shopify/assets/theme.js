...
(function (bodyScrollLock, MicroModal, Rellax, Flickity, themeCurrency, Sqrl, themeAddresses, axios, FlickityFade, Poppy, ellipsed, FlickitySync, AOS) {
  ...
  const searchPopdown = {
    onLoad() {
      ...
      const triggers = this.container.querySelectorAll(`[${selectors$B.popdownTrigger}]`);
      for (var i = 0; i < triggers.length; i++) {
        	console.log(triggers[i]);
          sections$g[triggers[i].id] = new SearchPopdownTriggers(triggers[i]);     
      }           
  ...
  class SearchPredictive {
    ...
    render(terms) {
      ...
      const serialized = `/search/suggest.json?q=${terms}&resources[type]=${resources}&resources[options][unavailable_products]=last&resources[options][limit]=9999`;
      fetch(serialized)
        .then(this.handleErrors)
    ...
    injectOther(results) {
      ...
      if (results.collections && results.collections.length) {
        ...
        results.collections.forEach((p) => {
          let collectionItem = p;
          if (collectionItem.body && collectionItem.body.length) {
            var str = collectionItem.body;
            var regex = /(http|ftp|https):\/\/([\w\-_]+(?:(?:\.[\w\-_]+)+))([\w\-\.,@?^=%&amp;:/~\+#]*[\w\-\@?^=%&amp;/~\+#])?/;
            
            // set url if regex returns value
            if(str.match(regex) !== '' && str.match(regex) !== undefined && str.match(regex) !== null) {
              var url = str.match(regex)[0];
            } else {
              var url = '';
            }
            
            //check if regex returns value, otherwise return empty string for template
            if(url !== '' && url !== undefined && url !== null) {
              collectionItem.image = url;
            } else {
              collectionItem.image = '';
            }
           
           //check if regex returns value, otherwise return empty string for template
           var cleanedString = str.replace(/<[^>]*>?/gm, '');
            
           if(cleanedString !== '' && cleanedString !== undefined && cleanedString !== null) {
              collectionItem.description = cleanedString;
            } else {
              collectionItem.description = '';
            }
            
          }  
          formattedCollections.push(collectionItem);
        });
      ...
      if (results.pages && results.pages.length) {
        ...
        results.pages.forEach((p) => {
          let pageItem = p;
          if (pageItem.body && pageItem.body.length) {
            var str = pageItem.body;
            var regex = /(http|ftp|https):\/\/([\w\-_]+(?:(?:\.[\w\-_]+)+))([\w\-\.,@?^=%&amp;:/~\+#]*[\w\-\@?^=%&amp;/~\+#])?/;
           ...
           if(cleanedString !== '' && cleanedString !== undefined && cleanedString !== null) {
              pageItem.description = cleanedString;
            } else {
              pageItem.description = '';
            }
            
          }  
          formattedPages.push(pageItem);
        });
    ...
    injectProduct(products) {
      ...
      products.forEach((p) => {
        ...
        if(product.body !== '' && product.body !== undefined && product.body !== null) {
          var str = product.body;
          var cleanedString = str.replace(/<[^>]*>?/gm, '');
  ...
  class ProductAddForm {
    ...
    linkForm() {
      console.log("Linking form:", this.productForm);
